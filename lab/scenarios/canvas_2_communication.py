...
class Canvas2CommunicationScenario(ScenarioRunner):
    def __init__(self) -> None:
        super().__init__(
            ScenarioMetadata(
                scenario_name="canvas_2_communication",
                canvas="2",
    ...
    async def run(self) -> None:
        ...
        if not infra_ok:
            for name in CANVAS_1_ASSERTIONS + CANVAS_2_ASSERTIONS:
                self.assert_true(name, False, infra_details)
            return

        canvas1 = _run_harness("canvas_1_infrastructure_harness/Canvas1.csproj")
        if canvas1 is None:
        ...
        canvas2 = _run_harness("canvas_2_communication_harness/Canvas2.csproj")
        if canvas2 is None:
...
def _run_harness(relative_csproj: str) -> dict | None:
...
def create_scenario() -> ScenarioRunner:
    return Canvas2CommunicationScenario()
