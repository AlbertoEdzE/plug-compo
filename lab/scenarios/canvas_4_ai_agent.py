...
from synthesizers.conversation_synthesizer import ConversationSynthesizer


class Canvas4AiAgentScenario(ScenarioRunner):
    def __init__(self) -> None:
        super().__init__(
            ScenarioMetadata(
                scenario_name="canvas_4_ai_agent",
                canvas="4",
                components_exercised=[
                    "KSquare.AgentOrchestrator",
    ...
    async def run(self) -> None:
        ...
        if not infra_ok:
            for name in CANVAS_1_ASSERTIONS + CANVAS_2_ASSERTIONS + CANVAS_3_ASSERTIONS + CANVAS_4_ASSERTIONS:
                self.assert_true(name, False, infra_details)
            return

        seed = int((os.environ.get("CANVAS_SEED") or "42").strip() or "42")
        payload = ConversationSynthesizer(seed=seed).payload()

        env = dict(os.environ)
        env["CANVAS_SEED"] = str(seed)
        env["CANVAS4_SYNTH_JSON"] = json.dumps(payload)
