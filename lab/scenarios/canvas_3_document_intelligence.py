...
from synthesizers.document_synthesizer import DocumentSynthesizer
from synthesizers.risk_synthesizer import RiskSynthesizer


class Canvas3DocumentIntelligenceScenario(ScenarioRunner):
    def __init__(self) -> None:
        super().__init__(
            ScenarioMetadata(
                scenario_name="canvas_3_document_intelligence",
                canvas="3",
                components_exercised=[
                    "KSquare.DocumentExtraction",
                    "KSquare.DocumentClassification",
    ...
    async def run(self) -> None:
        ...
        if not infra_ok:
            for name in CANVAS_1_ASSERTIONS + CANVAS_2_ASSERTIONS + CANVAS_3_ASSERTIONS:
                self.assert_true(name, False, infra_details)
            return

        seed = int((os.environ.get("CANVAS_SEED") or "42").strip() or "42")

        doc_payload = DocumentSynthesizer(seed=seed).payload()
        risk_payload = RiskSynthesizer(seed=seed + 1).payload()
        synth = {"seed": seed, **doc_payload, **risk_payload}

        env = dict(os.environ)
        env["CANVAS_SEED"] = str(seed)
        env["CANVAS3_SYNTH_JSON"] = json.dumps(synth)
...
def create_scenario() -> ScenarioRunner:
    return Canvas3DocumentIntelligenceScenario()
